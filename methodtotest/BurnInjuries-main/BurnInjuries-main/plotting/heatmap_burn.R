...
# IMPORTANT FIX:
# In your install, Module_listGen3 is a data.frame, not a list.
# Build a proper module -> genes list.
if (is.data.frame(Module_listGen3_raw)) {
  # Expect columns: Module, Gene (your debug shows they exist)
...
# Gen3_ann typically has: Module, Function, Cluster, Module_color, position
# ----------------------------
# Make sure strings not factors
...
# ----------------------------
# Filter modules: >=20% abs response in >=1 sample
# ----------------------------
...
if (nrow(module_response) < 2) {
  stop("Too few modules after filtering. Try lowering min_pct_module_response (e.g., 10) or cutoff_display.")
}
...
# Rename rows to "Module.Function" like paper
...
# Drop TBD (optional, matches their behavior)
keep_func <- ann_sub$Function != "TBD"
module_response <- module_response[keep_func, , drop=FALSE]
ann_sub <- ann_sub[keep_func, , drop=FALSE]

if (nrow(module_response) < 2) {
  stop("After dropping TBD, too few modules. Set keep_func step OFF or lower filters.")
}

# Display cutoff (paper)
...
# ----------------------------
# Column annotation: time_bin (annotation only; does NOT reorder)
# ----------------------------
time_bin <- meta$time_bin
names(time_bin) <- rownames(meta)
time_levels <- unique(time_bin)
time_cols <- structure(rand_color(length(time_levels)), names=time_levels)
...
# ----------------------------
# Left row annotation: Function colors
# ----------------------------
Build a named palette mapping Function -> color
func_df <- unique(ann_sub[, c("Function", "Module_color")])
func_cols <- setNames(func_df$Module_color, func_df$Function)

left_ha <- rowAnnotation(
  Function = ann_sub$Function,
  show_annotation_name = FALSE,
  simple_anno_size = unit(0.25, "cm"),
  col = list(Function = func_cols)
)

# Row split by aggregate cluster
row_split <- NULL
if (rowSplit_flag) {
  row_split <- ann_sub$Cluster
}
...
# ----------------------------
# Plot as HEATMAP (not DOT) first — stable + matches your example
# Then we can switch to dots after it works.
# ----------------------------
# ----------------------------
# Build heatmap object (MUST exist before draw())
# ----------------------------
ht <- Heatmap(
  plot_mat,
  name = "% Response",
  ...
  cluster_rows = TRUE,
  cluster_columns = FALSE,  # keep your time ordering (since you already reordered columns)
  row_split = row_split,

  show_row_names = FALSE,
  show_column_names = FALSE,

  use_raster = FALSE  # safer for PDFs/viewers
)
...
png(png_file, width = 3200, height = 1600, res = 200)
draw(ht, heatmap_legend_side = "left", annotation_legend_side = "left")
dev.off()
message("[SAVED] ", png_file)

# PDF: vector + safer encoding
pdf(out_pdf, width = 22, height = 12, useDingbats = FALSE)
draw(ht, heatmap_legend_side = "left", annotation_legend_side = "left")
dev.off()
